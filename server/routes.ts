...
const fileFilter = (req: any, file: any, cb: any) => {
  const allowedMimes = ['video/mp4', 'video/avi', 'video/mkv', 'video/mov', 'video/wmv'];
  if (allowedMimes.includes(file.mimetype)) {
    cb(null, true);
  } else {
    cb(new Error('نوع الملف غير مدعوم. يرجى رفع ملف فيديو صالح.'), false);
  }
};
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/admin/courses/:courseId/lessons', isAuthenticated, isAdmin, upload.single('video'), async (req: any, res) => {
    try {
      const courseId = parseInt(req.params.courseId);
      const userId = req.user?.claims?.sub;
      
      if (!req.file) {
        return res.status(400).json({ message: "لم يتم رفع ملف الفيديو" });
      }
      ...
      res.status(500).json({ message: "فشل في رفع الدرس" });
    }
  });

  // Serve video files
  app.get('/api/videos/:filename', isAuthenticated, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "خطأ في تشغيل الفيديو" });
    }
  });

  // Update lesson video information
  app.put('/api/admin/lessons/:id/video', isAuthenticated, isAdmin, upload.single('video'), async (req: any, res) => {
    try {
      const lessonId = parseInt(req.params.id);
      const userId = req.user?.claims?.sub;
      ...
      if (req.file) {
        ...
        if (lesson.videoUrl) {
          ...
          if (oldFilename) {
            const oldPath = path.join(process.cwd(), 'uploads', 'videos', oldFilename);
      ...
      res.status(500).json({ message: "فشل في تحديث الفيديو" });
    }
  });

  // Delete lesson and its video file
  app.delete('/api/admin/lessons/:id', isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const lessonId = parseInt(req.params.id);
      ...
      console.error("Error deleting lesson:", error);
      res.status(500).json({ message: "فشل في حذف الدرس" });
    }
  });

  // Get lessons with upload status
  app.get('/api/admin/lessons/pending', isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const pendingLessons = await storage.getLessonsWithPendingUploads();
      res.json(pendingLessons);
    } catch (error) {
      console.error("Error fetching pending lessons:", error);
      res.status(500).json({ message: "Failed to fetch pending lessons" });
    }
  });
